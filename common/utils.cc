...
void im2col(float* m_im_data, unsigned m_channel, unsigned m_height, unsigned m_width,
            ...
            unsigned m_num_threads) {
    unsigned col_height = (m_height + 2 * m_padding_h - m_filter_height) / m_stride + 1;
    unsigned col_width = (m_width + 2 * m_padding_w - m_filter_width) / m_stride + 1;
    unsigned col_channel = m_channel * m_filter_height * m_filter_width;
    ...
    for(unsigned tid = 0; tid < m_num_threads; tid++) {
        threads.emplace_back(std::bind([&](const unsigned begin, const unsigned end,
                                           const unsigned tid) {
            for (unsigned i = begin; i < end; i++) {
                unsigned offset_w = i % m_filter_width;
                unsigned offset_h = (i / m_filter_width) % m_filter_height;
                unsigned im_c = i / m_filter_width / m_filter_height;
