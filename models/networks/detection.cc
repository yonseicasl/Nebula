...
// Initialize network.
void detection_t::init_network(const std::string m_network_config) {
...
void detection_t::init_data(const std::string m_data_config) {
...
// Run network.
void detection_t::run(const std::string m_output_weight) {
...
// Load batch data.
void detection_t::load_data(const unsigned m_batch_index) {
    // Load batch data.
...
// Print results.
void detection_t::print_results() {
    if(run_type == TEST_RUN) {
        // get network box
        // draw detection box
        cv::Mat src, dst;
        src = cv::imread(inputs[iteration], 1);
        cv::resize(src, dst, cv::Size(input_width, input_height), 0, 0, cv::INTER_LINEAR);
        
        // draw detection box        
        //detection_box->make_box(network->input_height, network->input_width);
        detection_box->x_left = 0, detection_box->x_right = 240, detection_box->y_bottom = 0, detection_box->y_top = 120;

        // Draw horizontal line.
        for(unsigned i = 3 * detection_box->x_left; i < 3 * detection_box->x_right; i++) {
            dst.data[i + detection_box->y_bottom * input_width * 3] = 0.0;
            dst.data[i + detection_box->y_top * input_width * 3] = 0.0;
        }

        // Draw vertical line.
        for(unsigned i = detection_box->y_bottom; i < detection_box->y_top; i++ ) {
            dst.data[3*detection_box->x_left + i * 3 * input_width] = 0.0;
            dst.data[3*detection_box->x_left + 1 + i * 3 * input_width] = 0.0;
            dst.data[3*detection_box->x_left + 2 + i * 3 * input_width] = 0.0;
            dst.data[3*detection_box->x_left + 3 + i * 3 * input_width] = 0.0;

            dst.data[3*detection_box->x_right + i * 3 * input_width] = 0.0;
            dst.data[3*detection_box->x_right + 1 + i * 3 * input_width] = 0.0;
            dst.data[3*detection_box->x_right + 2 + i * 3 * input_width] = 0.0;
            dst.data[3*detection_box->x_right + 3 + i * 3 * input_width] = 0.0;
        }

        cv::imshow("prediction", dst);
        cv::waitKey();
