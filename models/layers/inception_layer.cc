...
inception_layer_t::inception_layer_t(network_t *m_network, layer_t *m_prev_layer, layer_type_t m_layer_type) :
...
inception_layer_t::~inception_layer_t() {
    delete [] workspace;
    delete [] bias;
    delete [] bias_update;
    delete [] weight;
...
void inception_layer_t::init(section_config_t m_section_config) {
    // Get layer settings.
    ...
    inception = this;
    for(unsigned i = 0; i < hops; i++) { inception = inception->prev_layer ? inception->prev_layer : NULL;}
    input_size = inception->output_size;

    input_height = inception->output_height;
    input_width =  inception->output_width;
    input_channel = inception->output_channel;
...
void inception_layer_t::init_weight(std::fstream &m_input_weight) {
...
void inception_layer_t::init_weight() {
...
void inception_layer_t::store_weight(std::fstream &m_output_weight) {
#ifdef GPU_ENABLED
...
void inception_layer_t::forward() {
    ...
    float *input_data = inception->output_data;
...
void inception_layer_t::backward() {
    // Gradient function
    gradient();
    ...
    float *input_data = inception->output_data;
    float *prev_delta = inception->delta;
...
void inception_layer_t::forward_batchnorm() {

}


void inception_layer_t::backward_batchnorm() {
